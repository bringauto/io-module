...
int arduino_mega_send_status_condition(const buffer current_status, const buffer new_status){
    bringauto::io_module_utils::DeviceStatus<bringauto::modules::io_module::ARDUINO_MEGA_INPUTS_COUNT,
                                             bringauto::modules::io_module::ARDUINO_MEGA_OUTPUTS_COUNT> c_status;

    int rc1 = c_status.deserializeFromBuffer(current_status);

    bringauto::io_module_utils::DeviceStatus<bringauto::modules::io_module::ARDUINO_MEGA_INPUTS_COUNT,
                                             bringauto::modules::io_module::ARDUINO_MEGA_OUTPUTS_COUNT> n_status;
...
int arduino_mega_aggregate_error(buffer *error_message, const buffer current_error_message, const buffer status){
    if(current_error_message.data == nullptr){ // In this case, there is no current error message in error aggregator - creating default error
        bringauto::io_module_utils::DeviceStatus<bringauto::modules::io_module::ARDUINO_MEGA_INPUTS_COUNT,
                                             bringauto::modules::io_module::ARDUINO_MEGA_OUTPUTS_COUNT> status_object;
